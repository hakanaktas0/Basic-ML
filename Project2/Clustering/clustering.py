...
def plot_images(images: np.ndarray, row_number: int, col_number: int) -> float:
...
def k_means_clustering(x: np.ndarray, k: int, similarity_function: Callable[[np.ndarray, np.ndarray], float], stop_threshold: int = 4) -> tuple[list, np.ndarray]:
...
centroids, membership = k_means_clustering(train_x, class_number, similarity_function)
cluster_to_class, matrix = find_cluster_class_pairs()
accuracy_percent = test_model(cluster_to_class)

print()
print("Matrix:\n%s" % matrix)
print("Cluster to class: %s" % cluster_to_class)
print("Accuracy: %.2f %%" % accuracy_percent)
